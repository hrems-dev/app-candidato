...
@section('title', 'Editar Atención o Taller')
@section('page-title', '📅 Editar Atención')
@section('page-description', 'Actualiza la información de esta actividad legal')

@section('content')
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 max-w-3xl">
    <form action="{{ route('admin.actividades.update', $actividad) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="titulo" class="block text-gray-700 dark:text-gray-300 font-bold mb-2">Título *</label>
            <input type="text" id="titulo" name="titulo" value="{{ old('titulo', $actividad->titulo) }}" required 
        ...
        <div>
            ...
            <textarea id="descripcion" name="descripcion" rows="6" required 
                class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500 dark:focus:border-blue-400">{{ old('descripcion', $actividad->descripcion) }}</textarea>
            @error('descripcion')
                <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="datetime-local" id="fecha" name="fecha" value="{{ old('fecha', $actividad->fecha->format('Y-m-d\TH:i')) }}" required 
                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500 dark:focus:border-blue-400">
                @error('fecha')
                    <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="text" id="ubicacion" name="ubicacion" value="{{ old('ubicacion', $actividad->ubicacion) }}" required 
                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500 dark:focus:border-blue-400">
                @error('ubicacion')
                    <p class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
        ...
        <div>
            ...
            @if($actividad->imagen)
            <div class="mb-4">
                <img src="{{ asset('storage/' . $actividad->imagen) }}" alt="Actividad actual" class="max-w-xs h-auto rounded-lg shadow-md">
